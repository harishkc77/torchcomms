...
RdmaTransport::~RdmaTransport() {
  // Cancel any scheduled progress() callback to prevent it from firing
  // with a dangling 'this' pointer after the transport is destroyed.
  if (progressTimeout_) {
    progressTimeout_->cancelTimeout();
  }
  // Drain any remaining pending works so their promises don't break.
...
// No-op: cleanup is deferred to the destructor.
